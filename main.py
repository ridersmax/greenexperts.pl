...
def resolve_user(request: Request, db: sqlite3.Connection) -> dict:
    email = (
        ...
        or request.cookies.get("nexa_user")
        or os.getenv("NEXA_DEFAULT_USER", "anna.kowalska@nexa.pl")
    )
    row = db.execute("SELECT * FROM employees WHERE email = ?", (email,)).fetchone()
    if row:
        return dict(row)
    admin = db.execute("SELECT * FROM employees WHERE system_role = 'Admin' ORDER BY id LIMIT 1").fetchone()
    if not admin:
        raise HTTPException(status_code=401, detail="Brak aktywnego użytkownika")
    return dict(admin)
...
@app.post("/api/session/{email}", tags=["System"])
def switch_session(email: str, request: Request, response: Response) -> dict:
    with get_db() as db:
        current = resolve_user(request, db)
    ...
    response.set_cookie("nexa_user", email, httponly=True, samesite="lax", max_age=86400)
...
def list_employees(request: Request) -> list[dict]:
    with get_db() as db:
        ...
        return [dict(row) for row in db.execute("SELECT * FROM employees ORDER BY employee_status, name").fetchall()]
...
def google_callback(code: str) -> RedirectResponse:
    client_id = os.getenv("GOOGLE_CLIENT_ID")
